...
case "$1" in
		...
		start-stop-daemon --start --quiet --pidfile /var/run/virtlogd.pid --exec /usr/sbin/virtlogd -- --daemon
                log_end_msg $?
                ;;
        stop)
                log_begin_msg "Stopping virtualization library daemon: libvirtd"
		start-stop-daemon --stop --quiet --retry 3 --exec /usr/sbin/virtlogd --pidfile /var/run/virtlogd.pid
		start-stop-daemon --stop --quiet --retry 3 --exec /usr/sbin/libvirtd --pidfile /var/run/libvirtd.pid
                log_end_msg $?
                rm -f /var/run/libvirtd.pid /var/run/virtlogd.pid
                ;;
        restart)
                $0 stop
                sleep 1
